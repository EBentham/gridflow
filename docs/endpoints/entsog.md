...
**Coverage:** Key UK gas border points — IUK, BBL, FRAN, IRL, NIRL, NORI
...
Daily physical gas flows at key UK interconnection points. Each row captures the volume of gas that physically crossed a border or transit point on a given day, in a specific direction. Used to track GB gas import/export balance, LNG send-out, and storage injection/withdrawal flows.
...
> Flow in GWh/day. `direction_key`: `entry` = gas entering GB; `exit` = gas leaving GB. Raw data may be in kWh/h or kWh/day — the transformer normalises all values to GWh/day. Deduplicated on `(timestamp_utc, point_key, operator_key, direction_key)`.
