...
| system_prices | **true vintage** (v0.21 unit L, PR #79; on-disk from L's rebuild) | `createdDateTime` → `published_at` | **Corrected 2026-09-07.** The former entry — "exposes no `publishTime`; honest fallback, not a gap" — read the absence of one field name as the absence of a vendor stamp. DISEBSP carries **`createdDateTime` on 94,415 / 94,415 raw rows**; it was simply unmapped, so the v0.20 Vintage Policy was reconstructing `available_at` where a vendor fact existed. Unit L maps it; the declared policy remains as a counted, logged fallback for rows lacking the field (expected 0). Note the vendor's own regime shift: ~52 min after period start in 2021-2023, ~24.7 h in 2024-2026 — so the change is **not** uniformly conservative against the retired 90-min assumption. |
