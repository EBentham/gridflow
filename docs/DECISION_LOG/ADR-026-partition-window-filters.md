...
future-coverage guarantee through NESO's own transformer base: every registered
`source == "neso"` transformer is a `GenericNesoJsonTransformer` subclass — most are
generated by `_make_transformer_class` from `ENDPOINTS`, and `carbon_intensity` is
special-cased to `CarbonIntensityTransformer`, which subclasses it — so a dataset
registered tomorrow inherits the exact-read automatically and cannot silently
reintroduce the fallback. The invariant is the shared base, not the factory — pinned cold-process (not in-process, which would pass on
