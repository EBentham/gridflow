...
**Date:** 2026-07-26 (amended same day, again 2026-08-03, and again 2026-08-22 — see the
...
## Amendment (2026-08-22, P0-a-1 / v0.19 "Silver Truth") — NESO exact-partition read, ruled OUTSIDE this ADR's two gated constants

**The measured defect.** 15 NESO datasets sat at exactly 80.0% duplicate silver rows —
286,680 of 286,977 removable duplicate rows across a 2026-08-01→08-05 extract
(`carbon_intensity`: 1,200 raw → 240 deduped; `regional_intensity`: 194,400 → 38,880). The
roadmap that opened this unit assumed the cause was NESO's absence from
`_PUBLICATION_WINDOW_FILTER_SOURCES` (this ADR's own filter registry) — **that premise is
falsified**, settled across five cross-model review passes on `P0-a-1-PLAN.md`.

**Why the filter-registry route is a no-op, not merely unhelpful.** Two routes were
evidenced, both dead ends:
- *Source-scoped* (`neso` added to `_PUBLICATION_WINDOW_FILTER_SOURCES`):
  `_resolve_publication_window_plan` resolves a NESO dataset's window through
  `silver/elexon/_publication_window.py::publication_window_params`, which looks the
  dataset up in the **Elexon** endpoint catalog. Every NESO dataset name is absent, so the
  lookup returns `None`, logged at **DEBUG** ("out of scope or exempt") — zero rows dropped,
  **zero warnings**, no partition ever examined.
- *Per-transformer* (`EVENT_WINDOW_FILTER = True`): the params are hardcoded
  `("periodStart", "periodEnd")`, while a NESO sidecar records only its own path-template
  variables (`from_dt`/`to_dt`/`date`/`period`). The window fails to resolve on **every**
  date including the first, producing five WARNINGs over a five-day run — not four, and
  still **zero rows dropped**.

Either way, the window that would apply to the batch's first day is the whole request
range, so even a resolving filter removes nothing. The natural experiment inside the
source confirms it: `intensity_date`/`intensity_period` use the identical generated
transformer class, are equally absent from the registry, and measured **zero** duplicate
rows over the same window — the only structural difference is that their connector chunks
one bronze partition per calendar day (`daily_iteration=True`), where the 15 duplicating
datasets batch up to 14 days into one partition (`_MAX_DAYS_PER_REQUEST`).

**The actual mechanism.** One bronze body, written under the batched window's first day,
was re-read by `_bronze_path_for_date`'s covering-partition fallback
(`_find_covering_bronze_partition`, ≤35-day lookback) from every target date the batch
covered, and each read re-emitted **all** of the body's rows into that date's own silver
file — one 5-day body producing 5 files, each holding all 5 days' rows.

**The fix, and why it is NOT a membership change to either constant this ADR gates.**
`GenericNesoJsonTransformer._bronze_files` (`silver/neso/carbon_intensity.py`) now reads
the exact date partition only, with no fallback — `neso` does **not** join
`_EXACT_PARTITION_ONLY_SOURCES`. Two reasons, both load-bearing:
1. That frozenset's other caller, `_bronze_date_dirs` (feeding
   `_available_at_from_bronze`), would return `[]` for any date without an exact
   partition and fall through to a **fabricated** `datetime.now(UTC)` vintage — reachable
   for `intensity_factors` (`reference_dataset=True`), whose own `_bronze_files` override
   ignores `_bronze_path_for_date` entirely and always returns non-empty.
2. `_EXACT_PARTITION_ONLY_SOURCES`'s own docstring states an invariant — "a
   correctly-fetched date either has its own exact partition or has no bronze at all" —
   that is true for `entsoe`/`entsog` (per-day chunking) and **false** for NESO, which
   deliberately batches up to 14 days per request. NESO's covering partition is
   legitimate; it must simply be read by exactly one date.

Scoping the fix to `GenericNesoJsonTransformer` instead buys the identical
future-coverage guarantee through NESO's own generated-class base: every current and
future NESO dataset is produced by `_make_transformer_class` from `ENDPOINTS`, so a
dataset registered tomorrow inherits the exact-read automatically and cannot silently
reintroduce the fallback — pinned cold-process (not in-process, which would pass on
ambient pytest collection imports) by
`tests/integration/test_neso_registry_coldstart.py`.

**No row filter, and no interval-semantics classification, for NESO.** Unlike this ADR's
CLOSED/HALF_OPEN machinery, the unit of exclusivity here is the bronze OWNER PARTITION,
not the row — no filter is applied at all. Correct, because NESO's forecast family
(`intensity_fw24h`/`fw48h`, `regional_intensity_fw24h`/`fw48h`, `*_pt24h`) is single-issue:
its event rows legitimately lie entirely outside the target day, and a target-day window
would set `all_dropped=True` for every date in a 3-day forecast tail, which
`pipeline/runner.py` treats as a hard failure for the whole range — trading a loud 5× for
a loud false failure on correct data.

**D-6 — covered-but-not-owned dates must be loud, never silently reported as "no bronze".**
After the exact-read fix, a target date genuinely covered by a batched partition but not
its owner returns `[]` from `_bronze_files`, and without a dedicated signal the base
`run()`'s generic "No bronze data" message would be **actively misleading** — bronze for
that date exists, owned elsewhere. `_bronze_files` resolves this on the miss path only
(never on the read path — pinned by
`test_covering_lookup_is_absent_from_the_success_path`), per BODY, never from the
partition's aggregate envelope:
- **Definite** WARNING when at least one body's OWN sidecar window (resolved via
  `request_window_from_sidecar`, this module's primitive — `partition_request_window`
  appears nowhere in this ruling) covers the target date: names the owner date, the
  covering window, and the remedial `gridflow transform neso <dataset> --start <owner>
  --end <owner> --reingest`.
- **Hedged** WARNING otherwise (no body resolves to a covering window — some
  resolved-but-non-covering, some unresolvable, or a mix): states explicitly that
  ownership is not confirmed and the rows may not exist, and carries the
  resolved-but-non-covering count plus every distinct failing `WindowReason` and its
  count.

Per-body resolution (not the envelope `partition_request_window` already provides) is
required because two bodies with disjoint windows can bridge a gap the envelope would
falsely claim to contain — a target date inside the gap is "contained" by
`min(start)..max(end)` although no single body covers it. Pinned by
`test_disjoint_windows_bridging_a_gap_is_hedged`, which fails against an envelope-based
implementation by construction. Every body is examined before deciding (never returns on
the first miss), so a hedge always carries complete evidence, unlike
`partition_request_window`'s own all-or-nothing short-circuit.

**Accepted residual, named.** `run()`'s generic "No bronze data" line still co-fires
alongside the D-6 hedge/definite message (no source-level suppression hook exists in the
base class for this). The NESO-specific line is the precise one; the generic one is
imprecise, not wrong. Fixing that co-firing is a base-class change affecting all six
sources — out of scope here.

**What this amendment does NOT cover.** The data half — deleting and re-transforming the
5× rows already on disk — ships separately as **P0-a-2**, gated on an explicit Bobbo
approval before any silver deletion. This amendment records the read fix only; the 5× rows
persist on disk until P0-a-2 runs. Partition **labelling** (a NESO silver file's filename
still does not describe its full contents — e.g. `carbon_intensity_20260801.parquet` now
correctly holds 08-01…08-05, the label matching the batch's first day rather than every
date it contains, the same shape this ADR's own `indo_20260710.parquet` precedent
already ships) is explicitly **not** fixed here — sized at ≥T3 in the P0-a-1 plan's D-5,
flagged for a future re-tiered unit, not attempted in this amendment.
