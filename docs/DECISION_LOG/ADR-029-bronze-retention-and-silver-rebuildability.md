...
MID, FUELHH, and system_prices. Rebuilding destination D requires retained bronze inputs
from D-1 and D, with each source's existing publication-window processing before ownership
selection. MID/FUELHH deduplicate combined prepared rows and overwrite D; system_prices
preserves owned per-body captures. A range rebuild therefore requires the preceding bronze
date as well as the requested dates. Missing bronze cannot be reconstructed by relabelling
another partition, and bronze remains immutable. Undeclared transformers retain their
existing derivation contracts.
