...
from gridflow.schemas.elexon import ElexonINDO
...
class INDOTransformer(BaseSilverTransformer):
    ...
    def transform(self, raw_df: pl.DataFrame) -> pl.DataFrame:
        ...
        if "published_at" in df.columns:
            ...
            # WHY: ElexonINDO declares published_at as a nullable contract column.
            # Emit it as typed-null when bronze lacks publishTime so the silver
            # schema is deterministic and partition globs don't drift across
            # history (a missing column breaks SELECT * reads spanning files that
            # do carry it).
