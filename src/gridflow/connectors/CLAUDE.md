...
These rules apply to anything under `src/gridflow/connectors/`. Read `base.py`
and one existing vendor (`elexon/` or `entsoe/`) before writing a new one.
...
- Always use the connector via `async with`. `__aenter__` is what initialises
  `_client` and `_semaphore`; bare instantiation leaves them `None`.
...
- **Rate limit:** `asyncio.Semaphore(self.config.rate_limit_per_second)` is
  ...
  (Note: it's a concurrency cap, not a strict per-second limit — fine for every
  current vendor.)
...
- A `CircuitBreaker` exists in `utils/retry.py` for orchestration but is **not**
  applied automatically by `BaseConnector`. Wire it in only if a vendor needs it.

## Auth patterns

- **Header auth (default):** `_auth_headers()` reads `api_key` + `api_key_header`
  from `SourceConfig` and returns the header dict. Used by `gie` (`x-key`).
...
## RawResponse fields (frozen dataclass)
...
Always set `data_date` when the API call is for a specific calendar date — the
bronze writer uses it for partitioning.
...
└── parsers.py         (optional) JSON/XML helpers (e.g. pagination, namespace stripping)
```

## Gotchas to copy from existing vendors

- **Elexon settlement-period endpoints (`PN`, etc.):** iterate periods 1–**50**,
  not 1–48. Spring forward has 46 periods, autumn back has 50. Stop early on
  HTTP error or HTTP 200 with empty `data: []` — this avoids writing 11-byte
  empty bronze files for periods 49–50 on non-DST days.
- **Elexon pagination:** every page increments `page`; stop when
  `current_page >= total_pages`. Helper in `connectors/elexon/parsers.get_pagination_info()`.
- **ENTSO-E auth:** the API key goes in **query params (`securityToken`)**, not
  a header. Override `_auth_headers()` and `__aenter__` together — leaving
  `_auth_headers()` returning the default would silently ship the key as a
  rejected header.
...
- **GIE auth:** `x-key` header (lowercase). Already handled by the default
  `_auth_headers()` when `api_key_header: "x-key"` is set in YAML.
- **Open-Meteo:** two base URLs (`api.open-meteo.com` for forecast,
  ...
  in `endpoints.py`, don't hardcode in `fetch()`.
